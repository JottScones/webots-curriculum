...
static float get_fitness() {
   ...
   */
   float avg_v, avg_v_diff, avg_sensor, avg_v_change;
   
   avg_v      = f_V / f_cpt;
   avg_v_diff = f_dv / f_cpt;
   avg_v_change = f_c / f_cpt; 
   avg_sensor = f_i / f_cpt;
   
   
   return avg_v  - avg_v_diff  - avg_sensor - avg_v_change;
}
...
static void state_run(void) {
  ...
  float tmp = 0;
  float f0 = prev_speed[0];
  float f1 = prev_speed[1];
  ...
  f_V += f_abs(prev_speed[0] + prev_speed[1]) > 0;
  f_dv += f_abs(prev_speed[0] - prev_speed[1]) / (prev_speed[0]+prev_speed[1]);
  f_i += tmp / MAX_IR_VALUE;
  f_c += f_abs(f0 - prev_speed[0])/(f0+prev_speed[0]) + f_abs(f1 - prev_speed[1])/(f1+prev_speed[1]);
  f_cpt++;
