...
static int run(void) {
  ...
  int left_obstacle = ps_value[4];
  int right_obstacle = ps_value[3];
  float delta = (left_obstacle - right_obstacle)/50;
  wb_motor_set_velocity(left_motor, -2-delta);
  wb_motor_set_velocity(right_motor, -2+delta);
