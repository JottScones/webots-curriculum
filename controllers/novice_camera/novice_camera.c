...
int main() {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    // Find the column (ie x value) with max sum
    delta = 0;
    max = 0;
    for (i = 0; i < width; i++) {
      if (max < intensity[i]) {
    ...
    if (index_max >= 0 && index_max < height) {
      ...
      iter = (MAX_SPEED * height) / (MAX_SPEED + BACKWARD_SPEED); //set speed = 0
    ...
    wb_motor_set_velocity(left_motor, SPEED_UNIT * (100 + K_TURN * delta));
    wb_motor_set_velocity(right_motor, SPEED_UNIT * (100 - K_TURN * delta));
  }
  free(intensity);
  wb_robot_cleanup();
