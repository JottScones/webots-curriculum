...
int find_middle(int tab[], int sizeTab) {
  ...
  // Dont need to locate middle if all pixels the same it is just the geometric centre
  if (identical) {
    free(copy);
    free(index_bests);
    return sizeTab / 2;
  ...
  // Take the top 10 pixel values and populate index_bests with their index
  for (i = 0; i < nb_best; i++) {
    int index = -1;
    int max = 0;
    for (j = 0; j < sizeTab; j++) {
  ...
  count = 0;
  for (i = 0; i < nb_best; i++) {
    // If the index is within 1/10th the width from the mean we keep it
...
int main() {
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    for (i = 0; i < width; i++)
      blue[i] = 255 - wb_camera_image_get_red(image, width, i, 0);
