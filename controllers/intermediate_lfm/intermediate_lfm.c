...
void lem(int array[], int size) {
  ...
  printf("[%d %d %d]\n",current_mean[0],current_mean[1],current_mean[2]);
...
void llm(int array[], int size) {
  ...
  all_white = !current_mean[0] && !current_mean[1] && !current_mean[2];

  if (!is_in[1] || all_white) {
    lfm_active = 0;
...
int utm_speed[] = {0, 0};
void utm(void) {
  utm_speed[RIGHT] = !lfm_active * -450;
  utm_speed[LEFT]  = !lfm_active * 150;
}
