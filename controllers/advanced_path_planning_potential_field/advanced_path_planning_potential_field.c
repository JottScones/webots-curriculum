...
static float compute_force(float pos_x, float pos_y, float *force_x, float *force_y) {
  ...
  if (d < rho_0 && d != 0 && d != -1) {
    // If puck is close enough to obstacle add repulsive force
...
static int run(void) {
  ...
  if (d_goal > 0.002) {
    // Set force values to net force in x and y direction
    ...
    // Direction robot needs to turn to align with force
