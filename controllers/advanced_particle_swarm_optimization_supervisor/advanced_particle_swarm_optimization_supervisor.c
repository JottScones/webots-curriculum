...
#define NB_RUN 30          // TODO: won't compile // the number of run
#define END_TIME 60000      // TODO: won't compile // the duration of each run [ms]
...
WbFieldRef orient_field[NB_ROBOTS];
...
static void reset(void) {
  ...
  for (; i < NB_ROBOTS; i++) {
    ...
    orient_field[i] = wb_supervisor_node_get_field(robots[i], "rotation");
...
static void random_position(void) {
  ...
  for (; n < NB_ROBOTS; n++) {
    ...
    // Set angle to previous angle
    o[3] = wb_supervisor_field_get_sf_rotation(orient_field[n])[3];
    
    wb_supervisor_field_set_sf_vec3f(trans_field[n], p);
    wb_supervisor_field_set_sf_rotation(orient_field[n], o);
  }
  step();
