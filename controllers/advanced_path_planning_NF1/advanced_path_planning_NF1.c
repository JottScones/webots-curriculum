...
static void next_cell(int x, int y, int *nx, int *ny, float *ntheta) {
  ...
  // Check surrounding cells move to the cell that exists, is closer and is not an obstacle
  if (d == 0) {
    *nx = x;
    *ny = y;
    *ntheta = 0;
...
static int run() {
  ...
  int nx, ny, nnx, nny;
  float ntheta,pre_ntheta;
  next_cell(x, y, &nx, &ny, &ntheta);
  pre_ntheta = ntheta;
  while (pre_ntheta == ntheta && d > 0) {
    next_cell(nx, ny, &nx, &ny, &ntheta);
  }
