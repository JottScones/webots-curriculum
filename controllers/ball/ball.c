...
int run(void) {
  ...
  if (autopilot) {
    ...
    zspeed += (float)rand() / (float)RAND_MAX * 0.02 - 0.01;

    /* respect speed limit */
    ensure_bounds(&xspeed, -RANDOM_SPEED_LIMIT, +RANDOM_SPEED_LIMIT);
    ensure_bounds(&zspeed, -RANDOM_SPEED_LIMIT, +RANDOM_SPEED_LIMIT);
  ...
  new_translation[1] = ball_translation[1];
  new_translation[2] = ball_translation[2] + tz;

  /* relative robot motion */
  // wb_custom_robot_move(tx, 0, tz, 0, 0, 0, 0);
