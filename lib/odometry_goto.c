...
void odometry_goto_step(struct sOdometryGoto *og) {
  ...
  float v_adapt = 200/ 0.13;  // conversion-factor for speed in [m/s] to e-Puck speed units
  ...
  float omega_adapt = 400/ (270 * PI / 180);  // conversion-factor for turn rate in [rad/s] to e-Puck speed units
  ...
  float v_e = v_c * v_adapt * (fabs(alpha_c) < 0.05);
